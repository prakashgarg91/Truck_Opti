...
class Dimensions:
    ...
    def fits_in_any_orientation(self, container: 'Dimensions') -> bool:
        ...
        return any(container.can_contain(orientation) for orientation in self.get_all_orientations())
...
class PackingPosition:
    ...
    def __post_init__(self):
        if any(coord < 0 for coord in [self.x, self.y, self.z]):
            raise DomainValidationError("Position coordinates cannot be negative")
...
class OptimizationStrategy(Enum):
    ...
    def from_string(cls, value: str) -> 'OptimizationStrategy':
        ...
        try:
            ...
            raise DomainValidationError(f"Invalid optimization strategy: {value}")
...
class CostBreakdown:
    ...
    def operational_cost(self) -> Money:
        ...
        return self.fuel_cost + self.driver_cost + self.maintenance_cost + self.loading_cost
