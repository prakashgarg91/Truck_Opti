...
class SecurityHeaders:
    """Helper for applying consistent HTTP security headers"""
    ...
    def get_default_headers(cls) -> Dict[str, str]:
        """Return base security headers, allowing config overrides"""
        headers = dict(cls.DEFAULT_HEADERS)
        custom_headers = current_app.config.get('SECURITY_HEADERS', {}) if current_app else {}
        headers.update(custom_headers)
        return headers

    @classmethod
    def apply_security_headers(cls, response):
        """Apply headers to the Flask response if not already set"""
