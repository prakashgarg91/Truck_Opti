...
from .entities import TruckEntity, CartonEntity, PackingResultEntity
from .value_objects import (
    OptimizationStrategy, Dimensions, Money,
...
class PackingDomainService:
        ...
        try:
            ...
            if not truck.is_available:
                return ServiceResult(
                    success=False, errors=["Truck is not available"])

            if not cartons:
                return ServiceResult(
                    success=False, errors=["No cartons to pack"])

            # Sort cartons by strategy
            sorted_cartons = self._sort_cartons_by_strategy(cartons, strategy)

            # Perform 3D packing
            packing_result = self._pack_cartons_3d(
                truck, sorted_cartons, strategy)

            if not packing_result:
                return ServiceResult(
                    success=False, errors=["Failed to pack cartons"])
    ...
    def _sort_cartons_by_strategy(
        ...
        if strategy == OptimizationStrategy.SPACE:
            ...
            return sorted(
                cartons,
                key=lambda c: c.volume.cubic_meters if c.volume else 0,
                reverse=True)
        elif strategy == OptimizationStrategy.WEIGHT:
            # Sort by weight (heaviest first for bottom placement)
            return sorted(
                cartons,
                key=lambda c: c.weight.kilograms if c.weight else 0,
                reverse=True)
        elif strategy == OptimizationStrategy.COST:
            # Sort by value (highest first)
            return sorted(
                cartons,
                key=lambda c: c.value.amount if c.value else 0,
                reverse=True)
        else:
    ...
    def _find_best_position(
            self,
            carton: CartonEntity,
            truck_dims: Dimensions,
            occupied_spaces: List[Dict],
            strategy: OptimizationStrategy) -> Optional[PackingPosition]:
        ...
        orientations = carton_dims.get_all_orientations() if carton.can_rotate else [
            carton_dims]
            ...
            for y in range(0, int(truck_dims.width), step_size):
                for z in range(0, int(truck_dims.height), step_size):
                    for orientation in orientations:
                        ...
                        if self._can_place_carton(
                                position, orientation, truck_dims, occupied_spaces):
                            # Score this position based on strategy
                            score = self._score_position(
                                position, orientation, strategy, carton)
    ...
    def _can_place_carton(
            self,
            position: PackingPosition,
            carton_dims: Dimensions,
            truck_dims: Dimensions,
    ...
    def _boxes_overlap(self, pos1: PackingPosition, dims1: Dimensions,
        ...
        return not (
            pos1.x +
            dims1.length <= pos2.x or pos2.x +
            dims2.length <= pos1.x or pos1.y +
            dims1.width <= pos2.y or pos2.y +
            dims2.width <= pos1.y or pos1.z +
            dims1.height <= pos2.z or pos2.z +
            dims2.height <= pos1.z)
    ...
    def _calculate_space_utilization(
            self,
            truck: TruckEntity,
            cartons: List[CartonEntity]) -> float:
        """Calculate space utilization percentage"""
        if not truck.volume or not cartons:
            return 0.0

        total_carton_volume = sum(
            c.volume.cubic_meters if c.volume else 0 for c in cartons)
        return min(
            100.0,
            (total_carton_volume /
             truck.volume.cubic_meters) *
            100)
    ...
    def _calculate_weight_utilization(
        ...
        total_carton_weight = sum(
            c.weight.kilograms if c.weight else 0 for c in cartons)
        return min(
            100.0,
            (total_carton_weight /
             truck.max_weight.kilograms) *
            100)
    ...
    def _calculate_optimization_score(
        ...
        packing_efficiency = (packed_count / total_count) * \
            100 if total_count > 0 else 0

        if strategy == OptimizationStrategy.SPACE:
            return (space_util * 0.6 + weight_util *
                    0.2 + packing_efficiency * 0.2) / 100
        elif strategy == OptimizationStrategy.WEIGHT:
            return (weight_util * 0.6 + space_util *
                    0.2 + packing_efficiency * 0.2) / 100
        else:
            return (space_util * 0.4 + weight_util *
                    0.4 + packing_efficiency * 0.2) / 100
...
class CostCalculationService:
    ...
    def calculate_total_cost(
            self,
            truck: TruckEntity,
            packing_result: PackingResultEntity,
        ...
        try:
            ...
            fuel_cost = self._calculate_fuel_cost(
                truck, distance_km, fuel_price)

            # Calculate driver cost
            driver_cost = truck.driver_cost_per_day or Money(
                800.0)  # Default daily rate
    ...
    def _calculate_insurance_cost(
            self, packing_result: PackingResultEntity) -> Money:
        """Calculate insurance cost based on cargo value"""
        total_value = sum(
            c.value.amount if c.value else 0 for c in packing_result.packed_cartons)
        return Money(total_value * self.default_insurance_rate)

    def compare_cost_scenarios(
            self, scenarios: List[Dict[str, Any]]) -> Dict[str, Any]:
                ...
                results.append(
                    {
                        ...
                        'cost_per_carton': cost_breakdown.total_cost.amount / len(
                            packing_result.packed_cartons) if packing_result.packed_cartons else 0,
...
class OptimizationService:
    ...
    def find_optimal_truck_combination(self,
                                       cartons: List[CartonEntity],
                                       available_trucks: List[TruckEntity],
                                       strategy: OptimizationStrategy) -> Dict[str,
                                                                               Any]:
        ...
        try:
            ...
            for truck in available_trucks:
                packing_result = self.packing_service.optimize_packing(
                    truck, cartons, strategy)

                if packing_result.success:
                    cost_result = self.cost_service.calculate_total_cost(
            ...
            if len(cartons) > 50:  # Only for large loads
                multi_truck_results = self._find_multi_truck_solutions(
                    cartons, available_trucks, strategy)
            ...
            if strategy == OptimizationStrategy.COST:
                ...
                results.sort(
                    key=lambda x: x['utilization_score'],
                    reverse=True)
            else:
    ...
    def _find_multi_truck_solutions(self,
                                    cartons: List[CartonEntity],
                                    available_trucks: List[TruckEntity],
                                    strategy: OptimizationStrategy) -> List[Dict[str,
                                                                                 Any]]:
        ...
        for truck_count in [2, 3]:
            for truck_combo in combinations(available_trucks, truck_count):
                solution = self._optimize_multi_truck_packing(
                    cartons, list(truck_combo), strategy)
                if solution:
                    multi_truck_results.append(solution)

        return multi_truck_results

    def _optimize_multi_truck_packing(self,
                                      cartons: List[CartonEntity],
                                      trucks: List[TruckEntity],
                                      strategy: OptimizationStrategy) -> Optional[Dict[str,
                                                                                       Any]]:
        ...
        try:
            ...
            for truck in trucks:
                ...
                packing_result = self.packing_service.optimize_packing(
                    truck, remaining_cartons, strategy)

                if packing_result.success and packing_result.data.packed_cartons:
                    # Remove packed cartons from remaining
                    packed_ids = {
                        c.id for c in packing_result.data.packed_cartons}
                    remaining_cartons = [
                        c for c in remaining_cartons if c.id not in packed_ids]
            ...
            if truck_assignments:
                ...
                avg_utilization = sum(
                    ta['packing_result'].optimization_score for ta in truck_assignments) / len(truck_assignments)

                return {
                    'trucks': [
                        ta['truck'] for ta in truck_assignments],
                    'packing_results': [
                        ta['packing_result'] for ta in truck_assignments],
