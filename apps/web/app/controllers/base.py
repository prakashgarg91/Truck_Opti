    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            try:
                ...
                missing_fields = [field_name for field_name in validation.required_fields 
                ...
                for field_name, expected_type in validation.field_types.items():
                    if field_name in data:
                        try:
                            if expected_type is int:
                                ...
                                data[field_name] = str(data[field_name]).lower() in ['true', '1', 'yes', 'on']
                            elif expected_type is list and isinstance(data[field_name], str):
                ...
                for field_name, validator in validation.custom_validators.items():
                    if field_name in data:
                        try:
...
def handle_exceptions(func):
    ...
    def wrapper(self, *args, **kwargs):
        try:
            ...
            return ControllerResult.failure(str(e), status_code=400).to_response()
        except BusinessLogicError as e:
            self.logger.warning(f"Business logic error: {str(e)}")
            return ControllerResult.failure(str(e), status_code=422).to_response()
        except EntityNotFoundError as e:
            self.logger.warning(f"Entity not found: {str(e)}")
            return ControllerResult.failure(str(e), status_code=404).to_response()
        except Exception as e:
