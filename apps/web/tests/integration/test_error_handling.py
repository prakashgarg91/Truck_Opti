...
def test_error_handling():
    ...
    try:
        ...
        if 'error' in result:
            ...
            print('   [WARNING] No error message returned')
    ...
    try:
        ...
        if 'error' in result:
            ...
            print('   [WARNING] No validation message returned')
    except Exception as e:
        print(f'   [ERROR] Exception occurred: {e}')
    ...
    try:
        ...
        print(f'   Status: HTTP {response.status_code}')
        print('   [SUCCESS] Invalid JSON properly rejected')
    except Exception as e:
    ...
    try:
        ...
        if 'error' in result or 'missing' in str(result).lower():
            print('   [SUCCESS] Missing fields properly validated')
        else:
            print('   [WARNING] Validation might be insufficient')
    except Exception as e:
        print(f'   [ERROR] Exception occurred: {e}')
    ...
    try:
        ...
        if 'error' in result or 'validation' in str(result).lower():
            print('   [SUCCESS] Negative quantities properly validated')
        else:
            print('   [WARNING] Negative quantities not validated')
    except Exception as e:
        print(f'   [ERROR] Exception occurred: {e}')
    ...
    try:
        ...
        if 'error' in result or 'overflow' in str(result).lower():
            print('   [SUCCESS] Large numbers properly validated')
        else:
            print('   [INFO] Large numbers accepted (might be valid)')
    except Exception as e:
        print(f'   [ERROR] Exception occurred: {e}')
...
Another_Invalid,100,200,300,invalid,5"""
        ...
        if 'error' in result or 'validation' in result.get('message', '').lower():
            print('   [SUCCESS] Invalid CSV data properly validated')
            print(f'   Validation errors: {result.get("validation_errors", "N/A")}')
        else:
            print('   [WARNING] CSV validation might be insufficient')
    except Exception as e:
        print(f'   [ERROR] Exception occurred: {e}')
