...
With Add/Edit/Remove functionality and comprehensive error logging
"""
...
# HTML Template (shortened for space)
HTML_TEMPLATE = '''
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tab { flex: 1; padding: 15px 20px; text-align: center; background: none; border: none; cursor: pointer; font-weight: 500; transition: all 0.3s; }
        .tab.active { background: white; color: #667eea; border-bottom: 3px solid #667eea; }
        .tab-content { padding: 30px; display: none; }
        .tab-content.active { display: block; }
        .btn { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; padding: 12px 24px; border-radius: 10px; cursor: pointer; font-size: 14px; font-weight: 500; transition: transform 0.2s; }
        ...
        input, select { width: 100%; padding: 12px; border: 2px solid #e9ecef; border-radius: 10px; font-size: 14px; transition: border-color 0.3s; }
        input:focus, select:focus { outline: none; border-color: #667eea; }
        .truck-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-top: 20px; }
        .truck-card { background: white; border: 2px solid #e9ecef; border-radius: 15px; padding: 20px; transition: border-color 0.3s; }
        .carton-list { background: white; border-radius: 10px; padding: 20px; margin-bottom: 20px; }
        .carton-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #eee; }
...
<body>
    <div class="container">
        ...
        <div class="main-card">
            ...
            <div id="trucks" class="tab-content active">
                ...
                <div class="form-group">
                    ...
                    <form onsubmit="addTruck(event)">
                        <div class="form-row">
                            <div><label>Truck Name:</label><input type="text" id="truck_name" required placeholder="e.g., Tata 14ft"></div>
                            <div><label>Length (cm):</label><input type="number" id="truck_length" required step="0.1" placeholder="427"></div>
                            <div><label>Width (cm):</label><input type="number" id="truck_width" required step="0.1" placeholder="183"></div>
                            <div><label>Height (cm):</label><input type="number" id="truck_height" required step="0.1" placeholder="213"></div>
                            <div><label>Max Weight (kg):</label><input type="number" id="truck_weight" required step="0.1" placeholder="10000"></div>
                            <div><label>Cost per KM (₹):</label><input type="number" id="truck_cost" required step="0.1" placeholder="25"></div>
                        </div>
            ...
            <div id="cartons" class="tab-content">
                ...
                <div class="form-group">
                    ...
                    <form onsubmit="addCarton(event)">
                        <div class="form-row">
                            <div><label>Carton Name:</label><input type="text" id="carton_name" required placeholder="e.g., Medium Electronics Box"></div>
                            <div><label>Length (cm):</label><input type="number" id="carton_length" required step="0.1" placeholder="40"></div>
                            <div><label>Width (cm):</label><input type="number" id="carton_width" required step="0.1" placeholder="30"></div>
                            <div><label>Height (cm):</label><input type="number" id="carton_height" required step="0.1" placeholder="25"></div>
                            <div><label>Weight (kg):</label><input type="number" id="carton_weight" required step="0.1" placeholder="5.0"></div>
                            <div><label>Value (₹):</label><input type="number" id="carton_value" required step="0.1" placeholder="250"></div>
                            <div><label>Quantity:</label><input type="number" id="carton_quantity" required min="1" placeholder="10"></div>
                            <div><label>Type:</label><select id="carton_type" required><option value="standard">Standard</option><option value="fragile">Fragile</option><option value="heavy">Heavy</option><option value="lightweight">Lightweight</option></select></div>
                        </div>
            ...
            <div id="optimize" class="tab-content">
                ...
                <div style="background: #e3f2fd; border-radius: 15px; padding: 25px; margin-top: 20px;">
                ...
                <div id="selected_cartons_display" style="background: #f1f8e9; border-radius: 15px; padding: 25px; margin-top: 20px; display: none;">
    ...
    <script>
            ...
            try {
                ...
                if (cartons.length === 0) {
                    ...
                    cartons.forEach((carton, index) => {
                        html += `
                            <div style="background: white; border-radius: 10px; padding: 15px; margin-bottom: 10px; border: 1px solid #ddd;">
                                <div style="display: flex; justify-content: between; align-items: center; gap: 20px;">
                                    ...
                                    <div style="display: flex; align-items: center; gap: 10px;">
                                        ...
                                        <input type="number" id="qty_${index}" min="1" value="${carton.quantity}" style="width: 80px; padding: 5px; border: 1px solid #ddd; border-radius: 5px;">
                                        <button onclick="selectCartonForOptimization(${index}, '${carton.name}', ${carton.length}, ${carton.width}, ${carton.height}, ${carton.weight}, '${carton.type}')" 
        ...
        function selectCartonForOptimization(index, name, length, width, height, weight, type) {
            ...
            const carton = { name, length, width, height, weight, type, quantity };
        ...
        function updateSelectedCartonsDisplay() {
            ...
            selectedCartonsForOptimization.forEach((carton, index) => {
                html += `
                    <div style="background: white; border-radius: 8px; padding: 12px; margin-bottom: 8px; border-left: 4px solid #4CAF50; display: flex; justify-content: between; align-items: center;">
                        <div>
                        ...
                        <button onclick="removeCartonFromOptimization(${index})" style="background: #f44336; color: white; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer;">
                            Remove
                        </button>
                    </div>
                `;
        ...
        async function editCarton(index) {
            try {
                ...
                document.getElementById('carton_quantity').value = 1;
        ...
        async function optimizeLoading() {
            ...
            document.getElementById('results').innerHTML = '<div style="text-align: center; padding: 40px;"><div style="border: 4px solid #f3f3f3; border-top: 4px solid #667eea; border-radius: 50%; width: 50px; height: 50px; animation: spin 1s linear infinite; margin: 0 auto 20px;"></div><p>Running advanced LAFF algorithm optimization...</p></div>';
...
def cartons_api():
    ...
    try:
        if request.method == 'GET':
            ...
            data = request.json
            app_logger.info(f"POST /api/cartons - adding {data.get('quantity', 1)} cartons: {data.get('name', 'Unknown')}")
            ...
            app_logger.info(f"Successfully added {data['quantity']} cartons")
            return jsonify({'success': True, 'added_count': data['quantity']})
            
    except Exception as e:
        log_error(error_logger, e, "cartons_api")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def optimize_api():
    ...
    try:
        ...
        if selected_cartons:
            # Convert selected cartons to Carton objects
            cartons_to_optimize = []
        ...
        for truck in trucks_db:
            packed_cartons, metrics = packer.optimize_with_rotations(cartons_to_optimize, truck)
        ...
        if not best_truck:
            app_logger.warning("No suitable truck found for optimization")
            return jsonify({'error': 'No suitable truck found for the given cartons'}), 400
