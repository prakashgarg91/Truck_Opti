...
| T-125 | Improve advanced 3D bin-packing algorithm quality | P0 | 🧠 Product | 🟡 Genetic packing now preserves shuffled order through the shared extreme-points path and uses Fisher-Yates shuffle; regression proof is `12/12`, including a new order-preservation fixture that differentiates `genetic` from plain `extreme_points`. Next quality step is broader heuristic benchmarking on harder mixed-load cases. |
| T-126 | Move packing algorithm execution to client side where required UX/perf needs it | P0 | 🏗️ Architecture | 🟡 Client-side execution now carries measurable proof for both recommendation and pack flows: the worker returns `duration` + `processedOn` metadata and `PackingPage` surfaces that local runtime evidence. Remaining work is bundle/perf tuning beyond proof. |
