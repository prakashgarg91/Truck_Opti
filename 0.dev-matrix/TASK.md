...
> **Evidence:** `cd frontend && npm run build` passes inside `npm run launch-check`; `cd frontend && npm audit` passes 0 vulnerabilities; `cd apps/web && npm audit` passes 0 vulnerabilities; `python -m pip_audit -r .\apps\web\requirements.txt` is at 0 known vulnerabilities; `npm run test:frontend-smoke` passes 17/17; `npm run test:public-smoke` passes 7/7 on 2026-04-22; `npm run test:prod-config` now passes 5/6 on 2026-04-22 with only Razorpay live readiness still failing; the Google sign-in button redirects correctly to `accounts.google.com` via the Supabase callback; and `npm run launch-check` passes 17/17 on the current tree on 2026-04-22.
...
| T-127 | Test all major paths and end-to-end flows, not just preflight gates | P0 | 🧪 Product | ✅ EXTENDED 2026-04-20: added `scripts/live-admin-proof.cjs` (all 7 /admin/* routes), `demo.admin` account seeding, and `cleanupProofCustomers()` for automated cleanup; `test:live-auth` + `test:live-admin` npm scripts added. Requires `SEED_DEMO_PASSWORD` env var to run. Earlier proof: authenticated E2E 2026-04-12 for admin+driver+agency+customer. |
