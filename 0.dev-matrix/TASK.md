...
>
> **Queue normalization note (2026-05-17 close-day):** the canonical machine-owned queue now lives in `0.dev-matrix/AI-TASKS.json`, and the current sellable launch-proof slice is complete. `TO-101` through `TO-106` are done, including `TO-104` evidence normalization. Treat the legacy `T-*` board below as detailed engineering history rather than the active launch queue; the next AI-owned work is the non-launch backlog (`GAP-01`, `GAP-02`, and parked `T-155`).
