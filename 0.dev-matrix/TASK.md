...
>
> **Queue normalization note (2026-05-18):** the canonical machine-owned queue now lives in `0.dev-matrix/AI-TASKS.json`, and the current sellable launch-proof slice is complete. `TO-101` through `TO-106` are done, including `TO-104` evidence normalization. The original 2026-05-16 design backlog (`GAP-01`, `GAP-02`) is now code-closed in the restored refactor slice; treat the next AI-owned work as the current visibility backlog in `0.dev-matrix/CURRENT-GAP-VISIBILITY.md` (`GV-01`, `GV-02`) plus parked `T-155`.
