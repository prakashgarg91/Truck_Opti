...
| T-154 | Restore clean-tree launch proof | P0 | 🧼 Governance | 🟡 The recurring start-of-day dirty-tree source is now fixed locally by ignoring and untracking `0.dev-matrix/test-reports/session-start-maintenance-status.json`; package this hygiene slice and rerun `npm run launch-check` to move the task back to green |
