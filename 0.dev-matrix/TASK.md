...
> **2026-04-10 security note:** the local audit surface is green across root, `frontend`, and `apps/web`, including after removing unused `frontend` Electron packaging. GitHub's default-branch alert count still sits at 2 moderate alerts after push, and the remaining 2 still need manual GitHub Security-tab review because `gh` is not authenticated in this workspace.
> **2026-04-10 Supabase note:** the project auth health endpoint and the Supabase MCP endpoint both return `401` without credentials, which confirms reachability. Agent-driven Supabase operations are still blocked only by the missing PAT/token and project-ref access in this session.
...
| T-131 | Reconcile GitHub Dependabot alert count with local audits | P1 | 🔐 Security | 🟡 Local root, `frontend`, and `apps/web` audits are green again on 2026-04-10, including after the unused frontend packaging cleanup; GitHub still reports 2 moderate alerts, and the remaining 2 still need manual GitHub Security-tab review |
...
| T-131 | Reconcile GitHub Dependabot alert count with local audits | P1 | Security | Check GitHub Security tab and any non-Node ecosystems after the 2026-04-09 `apps/web` fixes and 2026-04-10 `frontend` packaging cleanup; GitHub still reports 2 moderate alerts, but `gh` is not authenticated here |
