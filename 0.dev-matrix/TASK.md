...
| T-126 | Move packing algorithm execution to client side where required UX/perf needs it | P0 | 🏗️ Architecture | 🟡 Client-side execution now has measurable proof for both recommendation and pack flows, a dedicated `bench:packing` command for the smart recommendation path, and internal hot-loop tuning in `fitsAt(...)` plus the shared packing caches. Remaining work is browser-aligned perf/bundle tuning beyond these internal/core improvements. |
