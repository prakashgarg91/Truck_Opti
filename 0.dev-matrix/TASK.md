...
> **Evidence:** `cd frontend && npm run build` passed on 2026-04-09 inside `npm run launch-check`; `cd apps/web && npm audit` now passes 0 vulnerabilities after a lockfile-only refresh on 2026-04-09; `python -m pip_audit -r .\apps\web\requirements.txt` is back to 0 known vulnerabilities after bumping `cryptography` to `46.0.7`; `npm run test:frontend-smoke` passes 17/17 on 2026-04-09; `npm run test:prod-config` now passes 4/6 on 2026-04-09 after disabling PhonePe sandbox in Heroku; a Playwright live check now confirms the Google sign-in button redirects correctly to `accounts.google.com` via the Supabase callback; and `npm run launch-check` passes 17/17 on the clean committed tree.
> **2026-04-09 update:** repo-side dependency drift in `apps/web` is fixed again. The remaining external launch blockers are Razorpay test keys, missing `VITE_SENTRY_DSN`, pending migration push, and unverified authenticated browser flows.
> **2026-04-09 security note:** the local audit surface is now green across root, `frontend`, and `apps/web`, and the latest push dropped GitHub's default-branch alert count from 17 to 2 moderate alerts. The remaining 2 still need manual GitHub Security-tab review because `gh` is not authenticated in this workspace.
...
| T-131 | Reconcile GitHub Dependabot alert count with local audits | P1 | 🔐 Security | 🟡 Local root, `frontend`, and `apps/web` audits are green again on 2026-04-09, and pushing the fixes dropped GitHub's alert count from 17 to 2 moderate alerts; the remaining 2 still need manual GitHub Security-tab review |
...
| T-131 | Reconcile GitHub Dependabot alert count with local audits | P1 | Security | Check GitHub Security tab and any non-Node ecosystems after the 2026-04-09 `apps/web` audit fixes; the push reduced GitHub's count from 17 to 2 moderate alerts, but `gh` is not authenticated here |
