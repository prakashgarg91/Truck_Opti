...
| T-125 | Improve advanced 3D bin-packing algorithm quality | P0 | 🧠 Product | 🟡 Shared client-side packer extraction is done; next quality step is heuristic improvement from the new single source of truth in `frontend/src/lib/packing.ts` |
| T-126 | Move packing algorithm execution to client side where required UX/perf needs it | P0 | 🏗️ Architecture | 🟡 Client-side execution is now cleaner: both the page fallback and the Web Worker use the shared frontend packing module; deeper perf/regression evidence is still pending |
