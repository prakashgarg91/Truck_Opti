...
> **2026-04-13 security note:** the local audit surface is still green across root, `frontend`, `apps/web`, and both tracked Python requirement files, but user-reported GitHub default-branch alerts are back up to 17. Docker Scout now exposes a likely blind spot that local npm/pip audits miss: `node:20-alpine` reports 17 high CVEs and `python:3.11-slim` reports 4 high CVEs. `gh` is still unauthenticated here, so T-131 is reopened until exact remote manifest paths are verified.
...
| T-131 | Reconcile GitHub Dependabot alert count with local audits | P1 | 🔐 Security | 🟡 REOPENED — local npm audits are green for root, `frontend`, and `apps/web`, and both audited Python manifests are green, but user-reported GitHub default-branch alerts are back at 17; Docker Scout shows `node:20-alpine` with 17 high CVEs and `python:3.11-slim` with 4 high CVEs, while `gh` remains unauthenticated so remote manifest-path confirmation is still pending |
