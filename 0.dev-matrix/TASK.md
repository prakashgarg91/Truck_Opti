...
> **Evidence:** current synced head is `70e764c5`; `npm run launch-check` passed 14/14 checks on 2026-04-03; `npm run test:frontend-smoke` passed 16/17 on 2026-04-03; `npm run test:prod-config` passed 2/6 on 2026-04-03.
...
> **2026-04-03 close-day note:** next repo-side batch should remove the duplicated client-side packer split across `frontend/src/pages/PackingPage.tsx` and `frontend/src/workers/packingWorker.ts` before any deeper 3D heuristic work.

| ID | Task | Priority | Type | Status |
|----|------|----------|------|--------|
| T-124 | Frontend testing pass for key user-facing pages | P0 | 🧪 Product | 🟡 Full browser route audit completed: 47 routes exercised, and automated frontend smoke now passes 16/17 checks including both public registration wizards progressing to step 3; auth/contact flows still blocked by unreachable Supabase backend |
| T-125 | Improve advanced 3D bin-packing algorithm quality | P0 | 🧠 Product | 🟡 Next repo-side move: extract the duplicated packer shared by `PackingPage.tsx` and `packingWorker.ts`, then tune heuristics from one source of truth |
| T-126 | Move packing algorithm execution to client side where required UX/perf needs it | P0 | 🏗️ Architecture | 🟡 Web Worker path is already live, but the page and worker still duplicate engine logic; shared-module extraction should happen before deeper perf/regression work |
| T-127 | Test all major paths and end-to-end flows, not just preflight gates | P0 | 🧪 Product | 🟡 Public route map and key unauthenticated interactions are now exercised; authenticated end-to-end still blocked by auth backend reachability |
