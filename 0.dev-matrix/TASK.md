...
>
> **TO-112 production-readiness mission (2026-09-11 — queued, NOT started):** owner-pasted mission brief saved verbatim at `0.dev-matrix/MISSION-PRODUCTION-READY.md`. Goal: turn the restored local-first deployment (Heroku v5, `3d330d2e`) into a secure fully-functional production app — verified completion inventory, Supabase restore-or-replace, production auth matrix (Supabase OAuth Google login replaces client-decoded GIS token for privileged authority), provider config, audit-script policy fixes, core workflows, payments, observability, security review, gates. First session step: run the Phase 1 baseline command list in the brief and record exact counts. Hard gates: no `supabase db push` / real payment / credential rotation without explicit owner approval; VITE_* changes require redeploy. Status: queued — owner said "continue later".
>
> **TO-111 close-day (2026-06-10 — Copilot):** All originally broken production pages fixed on Heroku v94. Eight Supabase edge functions deployed (admin-portal-dashboard, payouts, agencies, contact, drivers, users, agency-portal-dashboard, agency-portal-drivers). Five demo accounts seeded (demo.admin, demo.driver, demo.agency, demo.customer, demo.razorpay). Google Maps API key updated and configured with referrers in Google Cloud Console — `/tracking` now renders India map. Browser verification confirms `/admin`, `/admin/users`, `/admin/payouts`, `/admin/drivers`, `/admin/agencies`, `/routes`, `/tracking` all load correctly with data instead of spinners. Launch-check 18/18 PASS. GitHub pushed (commit `3222e5bd`). Remaining open: TO-109 (4-digit OTP migration), TO-110 (uncommitted apps/web Python), T-116 (Sentry DSN), T-131 (Dependabot alerts). Owner blockers: Razorpay prod keys, Google OAuth smoke test, Twilio SMS, Supabase PITR.
>
> **TO-107 review-lane verdict (2026-06-07 17:55 IST — minimax-m3-free, re-reject):** `0.dev-matrix/frame-artifacts/TO-107/REVIEW-RESULT.md` (re-written 2026-06-07 17:55 IST) records `FINAL_REVIEW_VERDICT: changes-required`. The build submission is unchanged from the prior 17:46 rejection (still only `server.js | 2 ++` for `Pragma: no-cache` + `Expires: 0` in the SPA fallback), and the reviewer's freshest re-run of `d:/Github/Truck_Opti/.venv/Scripts/python.exe scripts/webwright/full_app_smoke.py` at `2026-06-07T17:55:07+0530` (raw stdout at `0.dev-matrix/test-reports/webwright-full-app-2026-06-07-to107-review-rerun/smoke.log`, `summary.json` re-written with `generated_at: "2026-06-07T17:55:07+0530"`) returned 13/13 PASS at the smoke-driver level but steps 04-08 (login, signup, forgot-password, terms, privacy) STILL return `TITLE=404 - Page Not Found / H1=Page not found` on `https://www.truckopti.in`. MCP blast-radius re-check confirms: `code-review-graph_get_minimal_context_tool` risk high architectural, 3 changed files, 0 affected flows for the bounded slice; `code-review-graph_get_impact_radius_tool` (max_depth=2) 8 nodes directly changed, 250 within 2 hops, 97 additional files affected; `code-review-graph_get_affected_flows_tool` 0 affected flows. Bounded-slice code audit remains sound (App.tsx routes declared, main.tsx BrowserRouter, server.js SPA fallback with the correct cache policy). TO-107 stays `active` in `0.dev-matrix/AI-TASKS.json`; the build lane must either (a) trigger a clean Heroku redeploy from a reviewed commit + re-smoke and confirm steps 04-08 resolve to owned pages, or (b) widen scope to fix `scripts/webwright/full_app_smoke.py` so the 5x 404 steps fail loudly. Only after the 5x 404 is actually closed on the live host should TO-107 move to a terminal status.
>
> **Last-Cycle Truth (2026-06-05 pre-review — minimax-m3-free):** the pre-completion pre-review at `0.dev-matrix/LAST-REVIEW.md` records `FINAL_REVIEW_VERDICT: changes-required`. The sellable launch-proof queue (`TO-101`..`TO-106`) is still `completed` in `0.dev-matrix/AI-TASKS.json`, but the live hosted surface regressed outside the scope of `npm test`: Webwright 2026-06-01 caught five public routes 404 on `truckopti.in` (`/login`, `/signup`, `/forgot-password`, `/terms`, `/privacy`) and the deployed login only exposes Google OAuth, which contradicts the launch checklist row 2.1. The new 4-digit OTP migration is committed in `c01d3fc1` but unapplied. `apps/web` has five uncommitted Python edits (`cost_engine.py`, `packer.py`, `routes.py`, `tests/test_drill_down_api.py`, `tests/test_enhanced_features.py`) that have not been re-validated. The launch-check last ran 2026-05-29 and was already 2/17 FAIL on a different dirty tree. Four new AI-executable follow-ups have been queued: `TO-107` (5 SPA route 404s), `TO-108` (login surface vs launch checklist), `TO-109` (apply 4-digit OTP migration on non-prod), `TO-110` (validate or park uncommitted `apps/web` changes). Owner-side blockers unchanged: live Razorpay production keys and `SEED_DEMO_PASSWORD`.
> **Queue normalization note (2026-05-18):** the canonical machine-owned queue now lives in `0.dev-matrix/AI-TASKS.json`, and the current sellable launch-proof slice is complete. `TO-101` through `TO-106` are done, including `TO-104` evidence normalization. The original 2026-05-16 design backlog (`GAP-01`, `GAP-02`) and the later visibility backlog (`GV-01`, `GV-02`) are now code-closed, including the final `AgencyJobsPage` and admin users/subscriptions reconciliations. Treat the next AI-owned work as clean-tree packaging plus new product slices or parked `T-155`, not more portal ownership discovery.
> **2026-05-17 (Copilot-079):** deployed the checkout root fix end-to-end: the five payment edge functions now canonicalize tier slugs to real `subscription_plans` UUIDs, and Heroku `v90` serves the matching checkout fallback bundle (`index-DH60AMI1.js`, `CheckoutPage-iUQRl1Nf.js`). Fresh unauthenticated `/checkout?plan=starter&billing=monthly` now redirects to `/login` instead of failing on plan lookup, but the shared authenticated returning-user page still enters `/checkout` and snaps back to `/pricing`, so `TO-105` / `TO-106` remain active and real-money proof stays on hold.
> **2026-05-17 (Copilot-080):** `TO-105` is now complete. A real production payment succeeded, `/subscription` shows active renewal through `17 Jun 2026` plus paid invoice `INV-202605-000007` for `₹499`, and Heroku `v91` now ships the billing UX follow-up: `PaymentCallbackPage` exposes `View billing history` and `SubscriptionPage` exposes a `Download` fallback when no hosted invoice PDF exists. Current billing truth is narrower: invoice rows are created in production, but no repo code currently populates `pdf_url` or sends invoice email, so hosted PDF/email delivery remains follow-up work rather than checkout proof.
> **2026-05-17 (Copilot-081):** `TO-106` and `TO-104` are now complete. Fresh production `npm run test:public-smoke` passes `12/12`, `https://www.truckopti.in/sw-v2.js` still serves `Cache-Control: no-cache, no-store, must-revalidate` plus `Service-Worker-Allowed: /`, and the root document also serves `Cache-Control: no-cache, no-store, must-revalidate`. Combined with the earlier real authenticated payment session, that is sufficient proof to close the stale-client lane and freeze the launch evidence into the handoff surfaces.
...
> **2026-05-13 (Copilot-070)**: T-153 and T-154 are now closed again on the current tree. `apps/web/requirements.txt` now pins `ujson==5.12.1`, `d:/Github/Truck_Opti/.venv/Scripts/python.exe -m pip_audit -r apps/web/requirements.txt` returns `No known vulnerabilities found`, `npm run graph:update` refreshes cleanly with no Graphify drift warning, and `npm run launch-check` is back to PASS (`17/17`) on the clean committed tree. Remaining launch blockers are now only T-110 live Razorpay keys and T-127 auth proof credentials/session.
> **2026-05-13 (Copilot-069)**: T-154 root cause is reduced further. The recurring git-dirty artifact from `resume-work` / session-start maintenance is now fixed locally by ignoring `0.dev-matrix/test-reports/session-start-maintenance-status.json` and `session-start-maintenance-*.log`, untracking the generated status JSON, and documenting the runtime-artifact boundary in `0.dev-matrix/GRAPHIFY.md`. Validation: `git ls-files` confirmed only the status JSON was tracked, `git status --short` reproduced the problem before cleanup, and `git rm --cached` removed the runtime file from the index. Human blockers unchanged: T-110 live Razorpay keys, T-127 `SEED_DEMO_PASSWORD` / proof credentials.
> **2026-05-12 (Copilot-068)**: T-126/T-127 maintainability moved forward with a shared proof-env contract instead of more speculative browser retries. Added `scripts/check-proof-env.cjs`, centralized validation in `scripts/_proofEnv.cjs`, routed `live-auth-proof`, `live-admin-proof`, and demo-account seeding through the shared validator, and added tracked `.env.proof.example`. Validation: `npm run check:proof-env` and `npm run check:proof-env:seed` now fail cleanly on the real missing vars, and `node --check` passes on all touched proof scripts. Human blockers unchanged: T-110 live Razorpay keys, T-127 `SEED_DEMO_PASSWORD` / proof credentials.
> **2026-05-12 (Copilot-067)**: attempted the remaining live browser measurement step for T-126 without reopening code. The current preview build redirects `/packing` to `/login`, and the dev-only quick-login path cannot run because `VITE_TEST_EMAIL` and `VITE_TEST_PASSWORD` are missing. Validation/evidence: live browser snapshot confirms the redirect, env presence check returned both vars missing, and `opencode` agreed bundle tuning should wait until auth/session is available. Human blockers now explicitly gate this step: valid authenticated browser session or owner-provided test credentials/session.
> **2026-05-12 (Copilot-066)**: T-126 progressed again with the remaining browser-side perf slice only. `frontend/src/hooks/usePackingWorker.ts` + `frontend/src/workers/packingWorker.ts` now use a persistent `requestId`-correlated worker router instead of per-call listener churn, `frontend/scripts/packing-benchmark.ts` now covers `16`/`64`/`128` truck recommendation workloads, and `frontend/src/pages/PackingPage.tsx` now shows end-to-end wall-clock plus browser-overhead timing for recommendation/pack runs. Validation: `cd frontend && npx eslint src/hooks/usePackingWorker.ts src/workers/packingWorker.ts` PASS, `cd frontend && npx eslint src/pages/PackingPage.tsx` PASS, `cd frontend && npm run bench:packing` PASS, `cd frontend && npm run build` PASS. Human blockers unchanged: T-110 live Razorpay keys, T-127 SEED_DEMO_PASSWORD.
> **2026-05-12 (Copilot-065)**: T-126 progressed with the next benchmark/perf step only. Added `bench:packing` (`frontend/scripts/packing-benchmark.ts` + `frontend/tsconfig.packing-benchmark.json`) to benchmark the smart recommendation path over a 16-truck candidate set, and tightened `frontend/src/lib/packing.ts::fitsAt(...)` with cached candidate/box bounds. Validation: `cd frontend && npm run test:packing` PASS (`13/13`), `cd frontend && npm run bench:packing` PASS, `cd frontend && npm run build` PASS. Human blockers unchanged: T-110 live Razorpay keys, T-127 SEED_DEMO_PASSWORD.
> **2026-05-12 (Copilot-064)**: T-125/T-126 progressed with the next two AI steps only. Added a harder mixed-load packing benchmark (`13/13` total) where `extreme_points` currently packs 6 items and seeded `genetic` packs 8, and optimized the packing hot path in `frontend/src/lib/packing.ts` by caching per-item rotations and per-runtime color lookup. Validation: `cd frontend && npm run test:packing` PASS (`13/13`), `cd frontend && npm run build` PASS, CRG incremental update PASS. Human blockers unchanged: T-110 live Razorpay keys, T-127 SEED_DEMO_PASSWORD.
> **2026-05-12 (Copilot-063)**: T-125/T-126 advanced locally with repo MCPs + `opencode`. `frontend/src/lib/packing.ts` genetic mode now preserves shuffled order and uses Fisher-Yates shuffle; `frontend/scripts/packing-regression.ts` is now `12/12` with a new order-preservation fixture. `frontend/src/workers/packingWorker.ts`, `frontend/src/hooks/usePackingWorker.ts`, and `frontend/src/pages/PackingPage.tsx` now surface real client-side timing/proof for recommendation runs. Validation: `cd frontend && npm run test:packing` PASS (`12/12`), `cd frontend && npm run build` PASS. Human blockers unchanged: T-110 live Razorpay keys, T-127 SEED_DEMO_PASSWORD.
> **2026-05-11 (Copilot-061 close-day)**: T-153 cleared (Graphify skill version synced to 0.4.18 in `.copilot` path — no more drift warning). T-124 AI-executable portion done (7/7 unit tests, 17/17 launch-check). Authenticated E2E (T-127) needs SEED_DEMO_PASSWORD (human). Next AI-executable tasks: T-125 (advanced 3D bin-packing quality), T-126 (client-side packing UX/perf).
> **2026-05-11 (Copilot-060 close-day)**: readiness batch committed (`c00b719c`, 28 files). Tree is clean. `npm run launch-check` → 17/17 PASS. devDep audit: 0 vulnerabilities. Session workflow rules W-1/W-2/W-3/W-4 now in RULES.md. CLOSING-DAY-HOOK updated with auto-commit rule. Next AI-executable task: T-153 (run `graphify install` to clear 0.4.15 vs 0.4.18 drift), then pick from project-progress top-3 (T-124 frontend testing, T-125 advanced packing, T-126 client-side packing).
...
> **Canonical queue snapshot (2026-05-17 close-day):** `TO-101` through `TO-106` are complete. No active launch-proof tasks remain in the canonical queue.
...
> **Current proof truth (2026-05-17 close-day):** a real authenticated production payment completes, `/subscription` shows the active renewal plus paid invoice, fresh public smoke against production passes `12/12`, and both `sw-v2.js` plus the root document serve `Cache-Control: no-cache, no-store, must-revalidate`. `TO-105`, `TO-106`, and `TO-104` are all complete for the current launch slice.
...
|----|------|----------|------|--------|
| TO-107 | Restore five production SPA routes on truckopti.in (`/login`, `/signup`, `/forgot-password`, `/terms`, `/privacy` all 404 against the live host) | P1 | 🔧 Validate | 🟡 ACTIVE 2026-06-05 pre-review, BUILD PASS REJECTED 2026-06-07 17:46 review lane AND 2026-06-07 17:55 re-review — see `0.dev-matrix/LAST-REVIEW.md` and `0.dev-matrix/frame-artifacts/TO-107/REVIEW-RESULT.md`. Bounded-slice code is sound (App.tsx routes declared, main.tsx mounts BrowserRouter, server.js SPA fallback with cache headers), but the 5x 404 still reproduces on the live host per the reviewer's re-run of `d:/Github/Truck_Opti/.venv/Scripts/python.exe scripts/webwright/full_app_smoke.py` at `2026-06-07T17:55:07+0530` (raw stdout at `0.dev-matrix/test-reports/webwright-full-app-2026-06-07-to107-review-rerun/smoke.log`). MCP blast-radius re-check confirms 0 affected flows for the bounded slice and high risk is architectural only (App.tsx is the routing god-node, not edited). Build lane must either (a) trigger a clean Heroku redeploy from a reviewed commit + re-smoke, or (b) widen scope to fix `scripts/webwright/full_app_smoke.py` so the 5x 404 steps fail loudly. |
| TO-108 | Reconcile deployed login surface with the launch checklist (deployed `/login` only exposes Google OAuth, contradicts `LAUNCH_CHECKLIST.md` row 2.1) | P1 | 🧭 Governance | 🟡 QUEUED 2026-06-05 pre-review; depends on `TO-107`; check `heroku config | grep VITE_AUTH_EMAIL_OTP_ENABLED` |
| TO-109 | Apply and exercise the 4-digit `job_offer` OTP migration on a non-prod Supabase project | P1 | 🗄️ Database | 🟡 QUEUED 2026-06-05 pre-review; migration `20260601223849` is committed in `c01d3fc1` and locked by `jobOfferOtpContract.test.ts` (6 vitest cases), but has not been applied to any Supabase project |
| TO-110 | Validate uncommitted `apps/web` Python changes (`cost_engine.py`, `packer.py`, `routes.py`, `tests/test_drill_down_api.py`, `tests/test_enhanced_features.py`) | P1 | 🧪 Quality | 🟡 QUEUED 2026-06-05 pre-review; dirty tree blocks clean-tree launch proof |
...
| T-125 | Improve advanced 3D bin-packing algorithm quality | P0 | 🧠 Product | 🟡 Genetic packing now preserves shuffled order through the shared extreme-points path, uses Fisher-Yates shuffle, and has a harder mixed-load benchmark in the regression lane. Current proof is `13/13`, including a benchmark that currently logs `extreme_points=6` vs `genetic=8` on the medium truck. Next quality step is broader end-to-end heuristic benchmarking. |
| T-126 | Move packing algorithm execution to client side where required UX/perf needs it | P0 | 🏗️ Architecture | 🟡 Client-side execution now has larger `bench:packing` recommendation baselines (`16`/`64`/`128` truck sets), a persistent `requestId`-routed worker hook instead of per-call listener churn, and live PackingPage end-to-end timing that separates worker duration from browser overhead. The next measured browser pass is currently blocked by auth/session prerequisites (`/packing` redirects to `/login`, `VITE_TEST_*` missing), so bundle or worker warm-start tuning should wait until an authenticated run is available. |
| T-127 | Test all major paths and end-to-end flows, not just preflight gates | P0 | 🧪 Product | ✅ EXTENDED 2026-05-12: authenticated proof scripts and demo-account seeding now share one proof-env validator through `scripts/_proofEnv.cjs`, `npm run check:proof-env` / `npm run check:proof-env:seed` expose the exact missing prerequisites up front, and tracked `.env.proof.example` documents the supported local env contract. Remaining gate is still human-side credential/session supply (`SEED_DEMO_PASSWORD` or equivalent test auth). |
...
| T-153 | Clear Graphify install drift warning | P2 | 🛠️ Tooling | ✅ DONE — `npm run graph:update` refreshes cleanly on 2026-05-13 (`436 nodes`, `514 edges`, `73 communities`) with no Graphify drift warning |
| T-154 | Restore clean-tree launch proof | P0 | 🧼 Governance | ✅ DONE — `npm run launch-check` passes `17/17` on 2026-05-13 after the session-start artifact untrack and `ujson` security bump |
...
| T-155 | Configure AWS SES invoice emails for hosted billing docs | P1 | 🔑 External | 🟡 PLANNED — hosted invoice PDFs already work on production, and the Supabase Edge billing flow now records `emailProvider='aws-ses'`; future work is to verify the SES sender/domain, set `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SES_REGION` (or `AWS_REGION`), optionally set `BILLING_EMAIL_FROM` / `BILLING_SUPPORT_EMAIL`, and rerun invoice backfill proof. |
