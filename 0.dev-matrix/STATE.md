...
| `DEPENDABOT-REMOTE-MISMATCH-20260413` | 🟡 WATCH | User-reported GitHub default-branch alerts are back up to 17 (5 high, 9 moderate, 3 low) even though full local npm audits are green at repo root, `frontend`, and `apps/web`, and both audited Python manifests are clean. Docker Scout now shows a plausible blind spot: `node:20-alpine` carries 17 high CVEs and `python:3.11-slim` carries 4 high CVEs. `gh` is unauthenticated here, so the exact remote alert manifest paths are still unverified. | MANAGER + OWNER |
