...
>
> **Current gap visibility (2026-05-18 — Copilot-083):** the parked `stash@{0}` refactor is now restored locally and the original 2026-05-16 gap pair is code-closed. Validation on the restored slice: `cd frontend && npm run build` PASS, `npm run glue:check` PASS (`0 gaps, 0 warnings`), and `npm run test:frontend-smoke` PASS (`50/50`) after aligning the `/subscription` smoke contract with the protected-route behavior. The current design backlog now lives in `0.dev-matrix/CURRENT-GAP-VISIBILITY.md`: the next two non-human gaps are `GV-01` partial trusted-backend coverage for agency/admin privileged operations and `GV-02` missing role-specific profile ownership for `/driver/profile` and `/agency/profile`.
