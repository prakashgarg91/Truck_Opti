...
export default function ForgotPasswordPage() {
    ...
    const resetMutation = useMutation({
        mutationFn: async () => {
            await authSupabaseApi.resetPasswordForEmail(identifier)
        },
        onSuccess: () => {
            setIsSent(true)
            toast.success('If the account exists, a password reset link is on its way.')
        },
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        const result = emailOrLoginIdSchema.safeParse(identifier)
        if (!result.success) {
            setIdentifierError(result.error.issues[0]?.message || 'Invalid email address or login ID')
            return
        }
    ...
    const isIdentifierValid = emailOrLoginIdSchema.safeParse(identifier).success

    return (
        <div className="p-6 animate-fade-in">
            <button
                onClick={() => navigate(`/login${modeParam}`)}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                <div className="animate-slide-up" style={{ animationDelay: '100ms' }}>
                    ...
                    <input
                        ...
                        className={`input text-lg font-medium ${identifierError ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''}`}
                    ...
                    {identifierError ? (
                        ...
                        <p className="mt-2 text-xs text-slate-500 flex items-center gap-1">
                            ...
                            Reset links work for seeded email addresses and login IDs.
                        </p>
                    )}
                </div>
                ...
                <button
                    type="submit"
                    disabled={!isIdentifierValid || resetMutation.isPending}
            ...
            {isSent && (
                <div className="mt-6 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl animate-fade-in">
                    <p className="text-sm font-medium text-emerald-700 dark:text-emerald-300">
                        Check the inbox for the email attached to that account and its spam folder if needed.
                    </p>
                </div>
            )}
            ...
            <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400 animate-fade-in" style={{ animationDelay: '300ms' }}>
                ...
                <Link to={`/login${modeParam}`} className="text-primary-600 hover:text-primary-700 font-semibold hover:underline">
                    Sign in
                </Link>
            </p>
        </div>
