...
Delegate coding/implementation work to remote sub-models (Kimi K2.7 via Ollama Cloud, GLM 5.2 via
z.ai's coding API) instead of doing the work by hand. The lead agent (Claude) should orchestrate,
provide context, and review/test results rather than hand-writing most of the implementation
itself.

CLI lives at `tools/subagents/subagent.py` in this repo. Run it directly
(`python3 tools/subagents/subagent.py ...`) or symlink it onto PATH as `subagent`:

```bash
ln -sf "$(pwd)/tools/subagents/subagent.py" /usr/local/bin/subagent
```

Requires `OLLAMA_API_KEY` and `GLM_API_KEY` env vars to be set wherever this runs.
...
subagent glm "implement a rate limiter in lib/rateLimiter.js with a 10/sec token bucket" --apply --base-dir .

# Big task spec from a file instead of shell-quoting
subagent kimi --prompt-file /tmp/task.md --files src/index.js --apply --base-dir .
